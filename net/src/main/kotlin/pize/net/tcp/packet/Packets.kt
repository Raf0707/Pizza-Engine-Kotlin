...
object Packets {
    ...
    fun getPacketInfo(data: ByteArray): PacketInfo? {
        return try {
            if (data.size < 2) return null
            val stream = PizeInputStream(ByteArrayInputStream(data))
            val packetID = stream.readByte().toInt()
            PacketInfo(packetID, stream)
        } catch (e: IOException) {
