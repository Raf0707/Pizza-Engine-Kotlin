...
class PrivateRSA {
    ...
    constructor(bytes: ByteArray?) {
        try {
            key = KeyFactory.getInstance("RSA").generatePrivate(PKCS8EncodedKeySpec(bytes))
            initCipher()
        } catch (e: Exception) {
            throw RuntimeException(e)
        }
    ...
    companion object {
        fun load(resource: Resource): PrivateRSA {
            try {
                resource.inStream().use { inStream -> return PrivateRSA(inStream.readAllBytes()) }
            } catch (e: Exception) {
                throw RuntimeException(e)
            }
        }
